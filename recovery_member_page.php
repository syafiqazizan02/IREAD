...
<?php
  ...
  else
  {
   header('location:./');
    ...
    $stmt = $conn->prepare("SELECT * FROM member WHERE member_ic=?");
    $stmt->bind_param("s", $user);
    $stmt->execute();
    $result = $stmt->get_result();
    $row = $result->fetch_assoc();
     $member_favQues= $row ['member_favQues'];
...
<body id="page-top">
  ...
  <header class="masthead ">
    <section id="contact">
      ...
      <h5 align="center"><?php echo $member_favQues;?></h5><br>
        <div class="row">
          <div class="col-lg-8 mx-auto">
            <form action="recovery_member_page.php" method="post" enctype="multipart/form-data" class="form-horizontal">
              <div class="control-group">
                <div class="form-group floating-label-form-group controls mb-0 pb-2">
                  <label></label>
                  <input class="form-control" name="member_favAnsw" type="text" placeholder="Enter Favourite Answer" required>
                </div>
              </div><br>
...
<?php
    	...
    	include('db_conn.php');
    		...
    		if(isset($_POST['recovery']))
    		{
    			...
    			$stmt = $conn->prepare("SELECT *FROM member WHERE member_favAnsw=?");
    			$stmt->bind_param("s", $member_favAnsw);
    			$stmt->execute();

    			if($stmt->fetch())
    			{
    			     echo "<script>window.location.href='recovery_member_password.php';</script>";
    			}
    			else
    			{
