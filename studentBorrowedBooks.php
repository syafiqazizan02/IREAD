...
<?php
  ...
  foreach ($displayname as $key)
  {
	...
	$boo = "SELECT book.book_tittle, borrow.borrow_date, borrow.due_date, returnss.book_id, returnss.borrow_id FROM returnss INNER JOIN book ON returnss.book_id = book.book_id INNER JOIN borrow ON returnss.borrow_id = borrow.borrow_id WHERE borrow.member_id = '".$key['member_id']."'";
