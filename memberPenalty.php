  ...
  foreach ($displayname as $key)
  {
    ...
    $boo = "SELECT book.book_code, book.book_tittle, borrow.borrow_date, borrow.due_date, done.done_date, `return`.penalty_day, `return`.penalty_amount
            FROM `return`
            ...
            WHERE borrow.member_id = '".$key['member_id']."' AND `return`.penalty_day > 0  AND `return`.penalty_amount > 0" ;
                  ...
                  <table id="bootstrap-data-table" class="table table-striped table-bordered">
                    ...
                    <tbody>
                      ...
                      foreach ($book as $key) {
                        ...
                        $borrow_date = date_format(date_create($key['borrow_date']), 'd M Y');
                        $due_date = date_format(date_create($key['due_date']), 'd M Y');
                        $done_date = date_format(date_create($key['done_date']), 'd M Y');
                        $penalty_day = $key['penalty_day'];
                        $penalty_amount = $key['penalty_amount'];
                        ...
                        <tr>
                          ...
                          <td><p style="color:red;"><b><?php echo $penalty_day ?> Days</b></p></td>
                          <td width="5%"><p style="color:red;"><b>RM <?php echo $penalty_amount ?></b></p></td>
                        </tr>
