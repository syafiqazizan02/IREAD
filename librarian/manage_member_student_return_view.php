<table id="bootstrap-data-table1" class="table table-striped table-bordered">
 ...
 <tbody>
   <?php
        ...
        $stmt5 = $conn->prepare("SELECT a.return_status as return_status, a.penalty_day as penalty_day, a.penalty_amount as penalty_amount, b.book_code as book_code, b.book_tittle as book_tittle, b.book_author as book_author, d.category_name as book_category,
        ...
        $stmt5->bind_param("ss", $member_id, $return_status);
        $stmt5->execute();
        $result5 = $stmt5->get_result();
          ...
          while($row5 = $result5->fetch_assoc())
          {
            ...
            $return_status = $row5['return_status'];
            $penalty_day= $row5['penalty_day'];
            $penalty_amount = $row5['penalty_amount'];
            $borrow_date = date_format(date_create($row5['borrow_date']), 'd M Y');
            $due_date = date_format(date_create($row5['due_date']), 'd M Y');
            $done_date = date_format(date_create($row5['done_date']), 'd M Y');
