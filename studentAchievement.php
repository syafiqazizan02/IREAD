...
<?php
  ...
  foreach ($displayname as $key)
  {
	...
	$achievement = "SELECT member.member_fullname, member.member_ic,  COUNT(done.done_id) AS Rank FROM done INNER JOIN member WHERE done.member_id = member.member_id GROUP BY done.member_id ORDER BY Rank DESC";
  $arc = mysqli_query($db, $achievement);

  // $achieve = "SELECT COUNT(done_id) as Gile FROM done WHERE member_id = '".$key['member_id']."'" ;
  // $arc = mysqli_query($db, $achieve);
?>
                ...
                <div class="card-body">
                  <table id="bootstrap-data-table" class="table table-striped table-bordered">
                    ...
                    <tbody>
                        <?php
                  			foreach ($arc as $key) {
                          $memberName = $key['member_fullname'];
                          $ranked = $key['Rank'];
                          $memberIc = $key['member_ic'];
                          $COUNT++;
                  			?>
                        ...
                        <tr>
                          ...
                          <td><?php echo $ranked ?></td>
                          <!-- <td><button onclick="nilamDescription()" >Click Here</button></td> -->
              			   </tr>
