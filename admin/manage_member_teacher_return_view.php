<table id="bootstrap-data-table1"  class="table table-striped table-bordered">
 ...
 <tbody>
   <?php
        ...
        $stmt14 = $conn->prepare("SELECT a.return_status as return_status, a.penalty_day as penalty_day, a.penalty_amount as penalty_amount, b.book_code as book_code, b.book_tittle as book_tittle, b.book_author as book_author, d.category_name as book_category,
        ...
        $result14 = $stmt14->get_result();
          ...
          {
            $book_code = $row14['book_code'];
            $book_tittle = $row14['book_tittle'];
            $book_category = $row14['book_category'];
            $book_author = $row14['book_author'];
            $return_status = $row14['return_status'];
            $penalty_day= $row14['penalty_day'];
            $penalty_amount = $row14['penalty_amount'];
            $borrow_date = date_format(date_create($row14['borrow_date']), 'd M Y');
            $due_date = date_format(date_create($row14['due_date']), 'd M Y');
            $done_date = date_format(date_create($row14['done_date']), 'd M Y');
