<?php
    ...
    $sql1 = 'SELECT MONTHNAME(librarian_register) as a FROM librarian 
    WHERE YEAR(librarian_register) = '.$_REQUEST["year"].'
    GROUP BY MONTH(librarian_register) ORDER BY MONTH(librarian_register)';
        $result1 = $conn->query($sql1);

    $sql2 = 'SELECT COUNT(MONTH(librarian_register)) as b FROM librarian
    WHERE YEAR(librarian_register) = '.$_REQUEST["year"].'
    GROUP BY MONTH(librarian_register) ORDER BY MONTH(librarian_register)';
