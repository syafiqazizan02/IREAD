<?php
  ...
  if (!$socket) { die("Socket Create Failed.\n"); }
    ...
    //Bind Socket
    if( !socket_bind($socket, "127.0.0.1" , 8077) )
    {
        ...
        if($errorcode==10048){
          ...
          echo "<script>window.open('manage_member_register_validation.php?id=$libraian_id&member_id=$member_id','_self')</script>";
           ...
           //Send some data
           socket_sendto($socket, $st, 8, 0, "127.0.0.1", 8077);
...
<?php
    ...
    $stmt4 = $conn->prepare("SELECT e.member_id as member_id,  e.member_ic as member_ic, e.member_fullname as member_fullname, e.member_email as member_email, e.member_contact as member_contact, e.member_address as member_address, e.member_contact as member_contact, f.type_name as type_name
                             FROM member AS e
                  ...
                  <div class="col-lg-12">
                    <div class="card">
                      <div class="card-header">
                        <small>Fingerprint Member</small>
                      </div>
                      ...
                      <div class="card-footer">
                          ...
                          <a  href="manage_member_register_socket_button.php?id=<?php echo $libraian_id?>&member_id=<?php echo $member_id?>" onclick="return confirm('Scaning Fingerprint?');"><button style="color:#ffffff;" class="btn btn-warning btn-xs" id="btn_register"><i class='fa fa-arrow-circle-o-right'></i>&nbsp;&nbsp;Register</button></a>
                      </div>
                    </div>
                  </div>
...
<?php
          ...
          echo "<script>window.open('manage_member_register_socket_finger.php?id=$libraian_id&member_id=$member_id','_self')</script>";
