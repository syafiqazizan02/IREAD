<?php
  ...
  if (!$socket) { die("Socket Create Failed.\n"); }
    ...
    //Bind Socket
    if( !socket_bind($socket, "127.0.0.1" , 8070) )
    {
        ...
        if($errorcode==10048){
          ...
          echo "<script>window.open('manage_member_register_validation.php?id=$libraian_id&member_id=$member_id','_self')</script>";
           ...
           //Send some data
           socket_sendto($socket, $st, 7, 0, "127.0.0.1", 8070);
...
<?php
          ...
          echo "<script>window.open('manage_member_delete_socket_finger.php?id=$libraian_id&member_id=$member_id','_self')</script>";
